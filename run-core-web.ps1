...
$root = $PSScriptRoot
$backendProj = Join-Path $root 'Core\OpenCRM.Core.Web\OpenCRM.Core.Web.csproj'
$uiDir = Join-Path $root 'Core\OpenCRM.Core.Web\Client'
...
Write-Host 'Avvio client Angular (npm run start)...' -ForegroundColor Cyan
$frontend = Start-Process -FilePath $npmCmd.Source -ArgumentList @('run','start') -WorkingDirectory $uiDir -PassThru -WindowStyle Normal
